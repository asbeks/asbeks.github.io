...
function rotateModels(models) {
  ...
  const slot = Math.floor(Date.now() / 300000) % models.length;
  return [...models.slice(slot), ...models.slice(0, slot)];
}
...
export async function callOpenCode(messages, options = {}) {
  ...
  const configuredModels = Array.isArray(options.models) && options.models.length ? options.models : FREE_MODELS;
  const orderedModels = apiKey ? configuredModels : rotateModels(configuredModels).slice(0, 3);
  ...
  for (const model of orderedModels) {
    ...
    if (attempt < orderedModels.length) await sleep(900);
  }

  const rateLimited = failures.some((x) => x.status === 429 || /rate limit|too many requests/i.test(x.message));
  const summary = rateLimited
    ? 'Available OpenCode free models are temporarily rate-limited.'
    : 'Available OpenCode free models are temporarily unavailable.';
  throw new ProviderUnavailableError(summary, failures);
}
