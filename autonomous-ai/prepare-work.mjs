...
async function model(messages, maxTokens = 2200, temperature = 0.1) {
  return callOpenCode(messages, { maxTokens, temperature, timeoutMs: 50000 });
}
...
async function listTool(root, rel = '') {
  ...
  return entries
    .filter((x) => !BLOCKED_PREFIXES.some((p) => `${safe ? `${safe}/` : ''}${x.name}`.startsWith(p)))
    .slice(0, 120)
...
async function writeFileTool(root, rel, content) {
  const safe = safeRelative(rel);
  if (/^(package-lock\.json|pnpm-lock\.yaml|yarn\.lock)$/i.test(safe)) throw new Error('Lockfile edits are not allowed.');
...
async function fetchIssue(owner, repo, number) {
  ...
  return {
    ...
    body: String(body.body || '').slice(0, 14000),
    labels: (body.labels || []).map((x) => x?.name).filter(Boolean),
  };
}

async function staticChecks(repoDir) {
...
async function qaPatch(issue, patch, checks) {
  const result = await model([
    { role: 'system', content: 'You are QA for an autonomous software contractor. Review only the PUBLIC issue, patch, and static-check results. Do not expose hidden chain-of-thought. Return JSON only: {"verdict":"pass"|"needs_work","summary":string,"risks":string}. Pass only when the patch plausibly satisfies the issue without unrelated or dangerous changes.' },
    { role: 'user', content: JSON.stringify({ issue, patch: patch.slice(0, 20000), staticChecks: checks }) },
  ], 1500, 0.05);
  const qa = parseJson(result.content);
  if (!qa) throw new Error('QA returned invalid JSON.');
  return {
...
const draftPath = path.join(workDir, 'draft-report.json');
...
try {
  ...
  // If a prior run created a patch but QA was throttled, resume from the saved patch.
  if (jobs.active?.id === selected.id && jobs.active?.stage === 'qa_retry') {
    const patch = await fs.readFile(patchPath, 'utf8');
    const draft = JSON.parse(await fs.readFile(draftPath, 'utf8'));
    patchSaved = true;
    const qa = await qaPatch(issue, patch, draft.staticChecks || []);
    const report = { ...draft, qa, preparedAt: new Date().toISOString(), patchPath: `autonomous-ai/work/${workId}/patch.diff` };
    ...
    jobs.active = {
      ...
      url: selected.url,
      stage: qa.verdict === 'pass' ? 'prepared' : 'needs_work',
      workId,
      preparedAt: report.preparedAt,
      qa,
      patchPath: report.patchPath,
    };
    jobs.status = qa.verdict === 'pass' ? 'work_prepared' : 'work_needs_improvement';
    jobs.lastError = null;
    jobs.retryAfterAt = null;
  } else {
    temp = await fs.mkdtemp(path.join(os.tmpdir(), 'autonomous-ai-job-'));
    ...
    for (let step = 0; step < MAX_STEPS; step += 1) {
      const result = await model(messages, 2400, 0.1);
      workerProvider = { mode: result.mode, model: result.model, attempts: result.attempts };
      const action = parseJson(result.content);
      ...
      let toolResult;
      try {
        if (action.action === 'list') toolResult = await listTool(repoDir, action.path || '');
        else if (action.action === 'read') toolResult = await readFileTool(repoDir, action.path);
        else if (action.action === 'search') toolResult = await searchTool(repoDir, action.query);
        else if (action.action === 'write') toolResult = await writeFileTool(repoDir, action.path, action.content);
        else throw new Error(`Unsupported worker action: ${action.action}`);
      } catch (toolError) {
        toolResult = `TOOL ERROR: ${clean(toolError instanceof Error ? toolError.message : String(toolError), 500)}. Use repository-relative paths; use "." only for listing the root.`;
      }
      messages.push({ role: 'user', content: `TOOL RESULT:\n${String(toolResult).slice(0, 16000)}` });
    }
    ...
    const checks = await staticChecks(repoDir);
    ...
    const qa = await qaPatch(issue, patch, checks);
    const report = { ...draft, qa, preparedAt: new Date().toISOString(), patchPath: `autonomous-ai/work/${workId}/patch.diff` };
    await fs.writeFile(path.join(workDir, 'report.json'), JSON.stringify(report, null, 2) + '\n');
  ...
  const retryAt = waiting ? new Date(Date.now() + 5 * 60 * 1000).toISOString() : null;
  jobs.active = {
    id: selected.id,
    title: selected.title,
    rewardUsd: selected.rewardUsd,
    url: selected.url,
    stage: waiting ? (patchSaved ? 'qa_retry' : 'retry_wait') : 'failed',
    workId,
    failedAt: waiting ? null : new Date().toISOString(),
    retryAfterAt: retryAt,
    error: waiting ? 'AI provider temporarily unavailable; automatic retry scheduled.' : clean(error instanceof Error ? error.message : String(error), 500),
  };
  jobs.status = waiting ? 'worker_waiting_provider' : 'worker_degraded';
  jobs.lastError = jobs.active.error;
  jobs.retryAfterAt = retryAt;
} finally {
